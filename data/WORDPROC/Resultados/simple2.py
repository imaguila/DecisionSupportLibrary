...
# ------------------------------------------------------------
# 6. PLOT: DEFAULT NON-TRANSPOSED VERSION
# ------------------------------------------------------------
...
if not TRANSPOSE:
    ...
    data_lens = df_compare[[SELECTED_PROFILE_NAME]].to_numpy(dtype=float)
    data_must = df_compare[["Student MUST"]].to_numpy(dtype=float)
    data_delta = df_compare[["Delta"]].to_numpy(dtype=float)
    ...
    fig, axes = plt.subplots(
        ...
        gridspec_kw={"width_ratios": [1.15, 1.15, 1.25]}
    )

    # --------------------------------------------------------
    # Panel A: Selected lens
    # --------------------------------------------------------
    ...
    axes[0].set_xticks([0])
    axes[0].set_xticklabels(["Domain"], fontsize=10)
    axes[0].set_yticks(np.arange(n_rows))
    axes[0].set_yticklabels(row_labels, fontsize=8)
    axes[0].set_ylabel("Requirement", fontsize=10)

    cbar0 = fig.colorbar(im0, ax=axes[0], fraction=0.12, pad=0.04)
    cbar0.set_label("Inclusion frequency (%)")

    axes[0].set_xticks(np.arange(-0.5, 1, 1), minor=True)
    ...
    cbar1 = fig.colorbar(im1, ax=axes[1], fraction=0.12, pad=0.04)
    ...
    cbar2 = fig.colorbar(im2, ax=axes[2], fraction=0.12, pad=0.04)
    ...
    fig.suptitle(
        "Requirement-level comparison: domain-specific relevance and human MUST priority",
        fontsize=14,
        fontweight="bold",
        y=1.01
    )
...
# ------------------------------------------------------------
# 7. OPTIONAL TRANSPOSED VERSION
# ------------------------------------------------------------

else:
    plot_df = df_compare[[SELECTED_PROFILE_NAME, "Student MUST", "Delta"]].copy()
    plot_df.index = row_labels
    plot_df = plot_df.T

    fig_width = max(14, 0.35 * len(row_labels))
    fig, ax = plt.subplots(figsize=(fig_width, 4.2))

    # Use only one colormap for the first two rows and divergent values are harder here.
    # This version is provided for exploratory purposes, but the non-transposed
    # version is recommended for the paper.
    ...
    ax.set_xticklabels(row_labels, rotation=65, ha="right", fontsize=8)
    ax.set_yticks(np.arange(3))
    ax.set_yticklabels([SELECTED_PROFILE_NAME, "Student MUST", "Delta"], fontsize=10)

    cbar = fig.colorbar(im, ax=ax, fraction=0.03, pad=0.02)
    cbar.set_label("Percentage / Delta")

    ax.set_title(
        "Transposed requirement-level comparison",
        fontsize=14,
        fontweight="bold"
    )
...
output_png = SCRIPT_DIR / "requirement_domain_vs_student_must.png"
output_jpg = SCRIPT_DIR / "requirement_domain_vs_student_must.jpg"
