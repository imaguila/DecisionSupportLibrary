...
def render_scatter( df, x, y, size=None, color=None, show_ids=False, key=None ) :
    text_column = None
    if show_ids:
        if "id" in df.columns:
            text_column = "id"
    ...
    hover_cols = [
        c
        for c in df.columns
        if not ( c.startswith("req_") or c.startswith("var_") or c.startswith("x_") )
    ]

    fig = px.scatter( df, x=x, y=y, size=size, color=color, text=text_column, hover_data=hover_cols )
